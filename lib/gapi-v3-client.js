...
const debug = debuglib('axm:client');
...
class gapiClient extends EventEmitter{
    ...
    async init(host, port, options, topicPrefix) {
        let self = this;   
        ...
        function getMQTTtopicSubscribeList(){
            let topicArray = [self._topicHeartBeat];
            for(let gw in self._GW){
                ...
                for(let node in self._GW[gw]['_dev']){
                    for(let handle in self._GW[gw]['_dev'][node]['_topics']){
        ...
        async function _mqttHeartbeatFunc() {
            ...
            if(self._MQTTclient.connected){
                let payload = {type:self._devType, date: Math.floor(Date.now()), id: self.clientID, iface: [10], seq: self._hbtCount}
                let hbtTopic = `${self._topicPrefix}${self.clientID}/0/8/0`;

                await self.mqttPublishAsync(hbtTopic, JSON.stringify(payload),{'retain': true}) 
        ...
        async function onConnectHandler() {
            ...
            let tList = getMQTTtopicSubscribeList()
        ...
        self._MQTTclient.on('message', async function (topic, message, packet) {
            ...
            let mqttMatch = gMQTTPattern.exec(self._topicFilter, topic);
    ...
    async createBleLink(gwid, timeout = gDefaultTimeout) {
        ...
        let lgwid = gwid.toLowerCase()
        if(! (lgwid in this._GW)) {
            this._GW[lgwid] = new gateway.gapiLink(this, lgwid, timeout);
