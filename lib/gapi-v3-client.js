...
class GapiClient extends EventEmitter {
  ...
  async init (host, port, options, topicPrefix, tls) {
    ...
    function _heartBeatHandler (mqttTopics, topic, message, packet) {
      ...
      if (message == '') { // LWT message
        ...
        if(self._GW[mqttTopics.id]){
          self._GW[mqttTopics.id]._gwConnectedState = false
        }
        self.emit('gw:disconnect', Object.assign({}, self._GWHBTList[mqttTopics.id]))
      } else {
        let msg = null
  ...
  async createBleLink (gwid, timeout = gDefaultTimeout) {
    ...
    if (!(lgwid in this._GW)) {
      ...
      debug({ log: null }, `[${lgwid}]:createBleLink :: SETTTING a new GAPI link with timeout: ${timeout}`)
      this._GWHBTList[lgwid].connected = true
      this._GW[lgwid]._gwConnectedState = true
    }
