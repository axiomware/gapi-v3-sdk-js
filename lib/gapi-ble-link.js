...
class gapiLink extends EventEmitter{
    constructor(clientObj, gwid, timeout) {
        super(); //event emitter
    ...
    async _dataHandler(mqttMatch, msgData, packet){
        if((mqttMatch.ch == 2) && (mqttMatch.addr.length > 0) && (mqttMatch.addr[0] == this.clientID)){
            ...
            if(packet['properties'] && packet['properties']['correlationData']) {
                let buf1 = Buffer.from(packet['properties']['correlationData']);
                let L = buf1.length;
                L = L > 4? 4:L;
            ...
            if(this._q[cdata]){
                ...
                let elapsedTime = endTime - this._q[cdata].stime;
                let f = this._q[cdata].res
    ...
    _gwAdvHandler(gwid, advData){
        ...
        advData.forEach((adv)=>{
            ...
            let cret = this._advCache.decode(adv)
    ...
    async _gwConnect() {
        if(!this._gwConnectedState){
            ...
            let tstr = `${this._mqttObj._topicPrefix}${this._gwid}/${this._iface_id}/`;
            ...
            try {
                ...
                let vret = await this.version(this._timeout);
                this._ver = vret;
                return true;
            } catch (err) {
                this._gwConnectedState = false;
    ...
    async _gwConnectBleDevice(node, handle, notificationFlag, connectFlag) {
        if(this._gwConnectedState){
            let ch = notificationFlag?'4':'5';
            
            let topicStr = `${this._mqttObj._topicPrefix}${this._gwid}/${this._iface_id}/${ch}/${node}/${handle}`;
            try {
    ...
    async _gwUnsubscribeTopicBleDevice(node, handle) {
        ...
        if(node == '' || node == null){//unsubscribe from all nodes and topics
            for(let node in this._topics.dev){
                for(let handle in this._topics.dev[node]){
    ...
    async _timeoutHandler(cdata, timeout){
        ...
        if(this._q[cdata]){
            let f = this._q[cdata].tmo
            await f()
        }
    } 

    async _enqueueRequest(resolve, reject, obj){
        let cdata = this._genCorrData();

        let fullTimeout = obj['tmo'] + this.linkTimeout;
        ...
        try {
            ...
            debug({log: obj}, `[${this._gwid}]:_enqueueRequest ::  Published request with cdata: ${cdata}`)
        } catch (err) {
            throw Error(err)
        }
    ...
    async version(timeoutInMilliseconds = this._timeout) {
        ...
        let vret = await this._gRequestResponse(pobj) 
        let ret = {
    ...
    async show(params, timeoutInMilliseconds = this._timeout) {
        let pclone = Object.assign({}, params);
        pclone['c'] = GAPIV3.C.GAPI_SHOW_CONNECTIONS_V3;
        pclone['tmo'] = timeoutInMilliseconds;
        let vret = await this._gRequestResponse(pclone)    
        let ret = {
    ...
    async getAdvInitialState(timeoutInMilliseconds = this._timeout) {
        let pclone = {
            'c': GAPIV3.C.GAPI_ADVERTISEMENT_GET_INITIAL_STATE_V3,
            'tmo': timeoutInMilliseconds
        }
        let vret = await this._gRequestResponse(pclone)    
        let ret = {
    ...
    async setAdvInitialState(params, timeoutInMilliseconds = this._timeout) {
        let pclone = Object.assign({}, params);
        pclone['c'] = GAPIV3.C.GAPI_ADVERTISEMENT_SET_INITIAL_STATE_V3;
        pclone['tmo'] = timeoutInMilliseconds;
        let vret = await this._gRequestResponse(pclone)    
        let ret = {
            't':vret.payload.tss + vret.payload.tsus*1e-6,
    ...
    async scanCacheRead(mode = 1, timeoutInMilliseconds = this._timeout) {
        ...
        vret.payload.nodes.forEach((adv)=>{
            let r = this._advCache.decode(adv);
            if(r) {
                nodes.push(r)
            }
        })
    ...
    scanLocalCacheLength() {
        let ret = this._advCache.length
        return(ret)      
    }

    scanLocalCacheClear() {
        let ret = this._advCache.clear()    
    }

    getScanLocalCacheTTL() {
        let ret = this._advCache.TTL    
    }
    ...
    async connect(node, params, timeoutInMilliseconds = this._timeout) {
        ...
        let tmp = Object.assign({}, node, params);
        let pclone = {
...
class bleDevice extends EventEmitter{
    ...
    async _gwSubscribeTopicBleDeviceLoc(handle, handler, notificationFlag) {
        let gwObj = this._gapiLinkObj;
        let node = this._nodeInfo.node;
        let ch = notificationFlag?'4':'5';  
        let topicStr4 = `${gwObj._mqttObj._topicPrefix}${gwObj._gwid}/${gwObj._iface_id}/4/${node}/${handle}`;
        let topicStr5 = `${gwObj._mqttObj._topicPrefix}${gwObj._gwid}/${gwObj._iface_id}/5/${node}/${handle}`;
    ...
    async reconnect(params, timeoutInMilliseconds = this._timeout) {
        ...
        let vret = await this._gapiLinkObj._gRequestResponse(pclone)   
